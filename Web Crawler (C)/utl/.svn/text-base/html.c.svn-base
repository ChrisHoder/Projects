...
// This function will get the next word, not in a tag in the given
// HTML string. A word is any string of consecutive letters.
// INPUT: HTML string, string to copy word into, position to start
//        in the string
// OUTPUT: copies word to given string, returns position of last character
//         in the word
...
/*
  ...
  1) IF (first call) THEN
     - skip the first two lines (url and depth)
  2) WHILE (character in HTML string) DO
     - IF (character is <) THEN
        - go to next > character
       ELSE
        - go to first letter
	- count from first letter to last letter
	- save characters between first letter and last letter to word string
	DONE
  3) return position of last letter
...
int GetNextWord(char *HTML,char* word,int pos){
  char c;
  //if first call
  if (pos == 0){
  ...
  while(c){
	...
	pos++;
      ...
      counter = pos;
      char *pt;
      //goes to first charcter outside the tag
      while (isALPHA((c = HTML[pos])) == 0){
	pos++;
	if (HTML[pos] == 0){
	  break;
	}
	if ((c = HTML[pos]) == '<'){
	  return GetNextWord(HTML,word,pos);
	  }
	
      }
      //find the end of the word
      counter = pos;
      while ( 0 != (c = HTML[counter])){
	if(isALPHA((c = HTML[counter])) == 1){
	  counter++;
	}
	else{
	  break;
      ...
      pt = &HTML[pos];
      //copies the word to the word string input
      strncpy(word,pt,(counter-pos));
...
// This checks to see if the given character is a letter or not by using comparisons of the ASCII value.
// INPUT: character
// OUTPUT: 1 if character, 0 if not a character
int  isALPHA(char c){
  if (!c){
    return 0;
  }
