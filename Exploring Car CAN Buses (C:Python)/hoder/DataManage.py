...
class DataManage:
    """
    This class will do the data Management for the CAN analysis. 
    This includes loading data up to the 
    MYSQL database, downloading data from the MYSQL database, 
    converting data to pcap, loading in packets.
    
    @todo: change the way to upload data to run bulk inserts 
           and not item by item as it is now.
    @todo: Allow for the use of both hexidecimal and decimal numbers 
           for all methods and provide better integration
    """
    ...
    def __init__(self, host, db, username, password, table, dataLocation):
        ...
        """ 
        This is the maximum time between 2 packets that we will 
        consider as a time delay between the two packets
         """
    ...
    def getInjectedLocation(self):
        ...
        @rtype: String
        @return: Path, relative to dataLocation, give in the constructor L{__init__},
                 where the injection data will be stored
        """
    ...
    def createTable(self, table):
        ...
        @type table: String
        @param table: Name of the new table in the database. 
                      The database information was set in the constructor, L{__init__}
        """ 
        self.table = table
        cmd =" CREATE TABLE `%s` ( \
  ...
  PRIMARY KEY (`id`) \
    ...
    def addData(self,cmd):
        """
        This method will insert data into the MYSQL database based on the given command 
        and the MYSQL information provided in the constructor, L{__init__}. 
        This method is designed for the insertion of data and not for retrieving data.
    ...
    def getData(self,cmd):
        """
        This method is designed to grab data from the MYSQL database and return it to the user. 
        It is not designed for insertions. See L{getData} for 
        ...
        @rtype: List of Lists
        @return: SQL data that you requested. The format will be a list of all the rows of data. 
    ...
    def addDataPacket(self,data,time,error):
        """
        This method will take in a data packet (such as one read from our sniff file) 
        and then upload it to the MYSQL database that is
        set for the class.
        
        @type data: List
        @param data: This is a list that is one packet as it read off by GooDFETMCPCAN 
                     rxpacket method. It is 14 elements long and each byte is stored to 
                     an element as an ASCII character .
        @type time: float
    ...
    def getCmd(self,packet,time,error,duration,filter, comment=None):
        """
        This method will create a sql insertion command based on the 
        given data and the MYSQL information stored in the class. This is 
        designed for inserting 1 packet.
        
        @type packet: Dictionary
        @param packet: This is a dictionary that contains the information 
                       for an entire data packet sniffed off of the CAN bus it has already
                       been parsed from the format that it is saved as in the sniff method 
                       from GoodFETMCPCANCommuniation, L{GoodFETMCPCANCommunication.sniff}. 
                       See L{parseMessageInt} to 
                       parse the message and for information on the packet Dictionary. 
                       The keys expected to be contained in this dictionary are as follows
                       sID, length,rtr,error, ide, eID (optional), db0 (optional), ... ,
                       db7 (optional). See the parsing method for information on all components
    ...
    def writeDataCsv(self,data, filename):
        """
        This method will write the given data to the given filename as a csv file. 
        This is designed to write packet data to files
        
        @type data: List of Lists
        @param data: A list of the data we wish to write to the csv file. 
                     The format is that each element in the list is a 
                     considered a row. and then each element in the row is a 
                     column in the csv file. This can handle both string and 
                     numeric elements in the lists. This is 
    ...
    def writePcapUpload(self,filenameUp,filenameWriteTo):
        """
        This method will create a pcap file of the data contained in a given csv file. 
        The csv file is assumed to be of the format
        saved by the sniff method in the GoodFETMCPCANCommunication class, 
        L{GoodFETMCPCANCommunication.sniff}. 
        
        @type filenameUp: String
        @param filenameUp: path/filename for the csv file that will be read and 
                           used for making a pcap file. This assumes that the 
                           .csv is included in the input.
        
        @type filenameWriteTo: String
        @param filenameWriteTo: path/filename for the pcap to be saved to. 
                                This assumes that there is the .pcap ending 
    ...
    def writeToPcap(self,filenameWriteTo, data):
        ...
        @type data: List of Lists
        @param data: A list of the data we wish to write to the csv file. 
                     The format is that each element in the list is a considered a row. 
                     and then each element in the row is a column in the csv file. 
                     This can handle both string and numeric elements in the lists. This is 
                     the format that is returned by L{opencsv}.
        """
        ...
        for line in data:
            ...
            # 8 data bytes
            msg += '%s%s%s%s%s%s%s%s' % (chr(line[8]), chr(line[9]), chr(line[10]), chr(line[11]), \
                                         chr(line[12]), chr(line[13]), chr(line[14]), chr(line[15]))
            # write message
            f.write(msg)
        f.close()
    ...
    def writetoPcapfromSQL(self, filenameWriteto, results): # pass in results from SQL query
        ...
        @type filenameWriteto: String
        @param filenameWriteto: path/filename for the pcap to be saved to. 
               This assumes that there is the .pcap ending included in the input.

        @type results: List of Lists
        @param results: A list of the data we wish to write to the csv file. 
                        The format is that each element in the list is a considered a row. 
                        and then each element in the row is a column in the csv file. This 
                        can handle both string and numeric elements in the lists. This is 
        ...
        for line in results:
         ...
         #   print int(bytes[0],16)
            # first 5 bytes
            msg = '%s%s%s%s%s' % (chr(int(bytes[0], 16)), chr(int(bytes[1], 16)), \
             chr(int(bytes[2], 16)), chr(int(bytes[3], 16)), chr(int(bytes[4], 16)))
            # 3 bytes of zero to provide stuffing for Wireshark
            msg += '%s%s%s' % (chr(int('00', 16)), chr(int('00', 16)), chr(int('00', 16)))
            # 8 data bytes
            msg += '%s%s%s%s%s%s%s%s' % (chr(int(bytes[5], 16)), chr(int(bytes[6], 16)), \
            chr(int(bytes[7], 16)), chr(int(bytes[8], 16)), chr(int(bytes[9], 16)), \
            chr(int(bytes[10], 16)), chr(int(bytes[11], 16)), chr(int(bytes[12], 16)))
    ...
    def parseMessage(self,data):
        """
        This method will parse a row of the data packets that is of the form of the packet 
        as returned by the GoodFETMCPCAN rxpacket method will simply convert
        each databyte to integers and then call L{parseMessageInt}. See this method for more information. 
        
        @type data: List
        @param data: This is a list that is one packet as returned by GoodFETMCPCAN 
                     rxpacket method. It is 13 bytes long and ASCII value.
        
        @rtype: Dictionary
        @return: This is a dictionary that contains the information for an entire data packet 
                 sniffed off of the CAN bus it has already
                 been parsed from the format that it is saved as in the sniff method from 
                 GoodFETMCPCANCommuniation. See L{parseMessageInt} to 
                 parse the message and for information on the packet Dictionary. 
                 The keys expected to be contained in this dictionary are as follows
                 sID, length,rtr,error, ide, eID (optional), db0 (optional), ... , db7 (optional). 
                 
                     1. sID: standard ID of the packet
                     2. length: length of the data packet (number of data bytes). This will be 
                        0 for an Remote Transmission Request frame
    ...
    def parseMessageInt(self,data):
        """
        @type data: List
        @param data: This is a list that is one packet as it read off by L{opencsv} method. 
                     The Elements will have been convereted to integers.
        ...
        @return: This is a dictionary that contains the information for an entire data packet
                 ...
                 sID, length,rtr,error, ide, eID (optional), db0 (optional), ... , db7 (optional). 
                     ...
                     1. sID: standard ID of the packet
                     2. length: length of the data packet (number of data bytes). 
    ...
    def packet2str(self,packet):
        """
        Converts the packet, in the form of the datapacket provided by GoodFETMCPCAN 
        rxpacket and each element is a string ASCII character corresponding to the Hex number
        of the databyte. This will convert the list to a string of the hex bytes.
        
        @type packet: List
        @param packet: This is a list that is one packet as it read off by the 
                       GoodFETMCPCANComunication class. It is 13 bytes long and 
                       string character is in each element.
        
        @rtype: String
        @return: String that is the data packet printed out where each byte 
                 has been converted to the hex characters.
        """
        toprint="";
        for bar in packet:
    ...
    def uploadData(self,filename):
        ...
        @param filename: This is the path to the file that the user wishes to upload. 
                         It is assumed to contain the .csv ending and
                         be a file in the format of the data that is saved by 
                         GoodFETMCPCANCommunication sniff method. 
        """
        ...
        for packet in data:
            ...
            try:
                ...
                #unable to add, raise an exception
                raise Exception, "Error inserting into db, %s with the command %s"\
                                                                   % (self.db, cmd)
    ...
    def opencsv(self,fileObj):
        """
        This method will load packet data from a csv file. The format of the data will 
        be of the form of the data that was saved by the
        GoodFETMCPCANCommunication sniff method. See L{GoodFETMCPCANCommunication.sniff} 
        for more information on the meaning of the databytes. 
        The elements in each row of the csv file will be parsed as if they are the following types:
                     1. float
                     2. string
                     3. integer 
                     4 Hex value (i.e. 'ff' or 'af')
        This will ignore any lines in the csv document that begin with # in the 
        first column. This allows for comments to be added.
        
        @type fileObj: Object of the file
        @param fileObj: This is the file object (i.e. what is returned by the open
                        command) of the csv document that is to be read.
        
        @rtype: List of lists
        @return: The csv file parsed by rows and columns. The format will be that each element
                 in the return list will be one row of the
                 csv file. All entries will be stored as a number except column 2 which will 
                 be stored as a string (see above).
        """
    ...
    def readWriteFileHex(self, filename):
        """
        This method will be used for reading a file for of packets to be written to the the CAN bus. 
        The format of the csv layout is expected to be as follows with each element stored as a hex 
            ...
            @type filename: String
            @param filename: path/filenae to the csv file to be uploaded. 
                             This is expected to include the .csv ending. The format of the rows
                             is described above. 
                             
            @rtype: List of Lists
            @return: This will simply turn the csv file to a list of lists where the 
                     elements of the outer list are rows in the csv file. Each row is a list 
                     of all the columns.  The time delay will be converted to a float while 
                     all other columns are converted to an integer.
        """
        try:
            fileObj = open(filename,'rU')
        except:
    ...
    def readWriteFileDEC(self,filename):
        """
        This method will be used for reading a file for of packets to be written to
        the the CAN bus. The format of the csv layout is
        expected to be as follows with each element stored as an integer value except
        for the time delay which is expected as a float:
            - col 0: delay time from previous row (0 if no delay)
            - col 1: Standard ID
            - col 2: Data Length (0-8) 
            - col 3: db 0
        ...
        Additionally, this can now take in the packets that the sniff method writes
        out, L{GoodFETMCPCANCommunication.sniff}. It will detect
        either the header that is added to these files or that the length is longer 
        than those written by the fuzz methods.
        
        @type filename: String
        @param filename: path/filenae to the csv file to be uploaded. This is
                         expected to include the .csv ending. The format of the rows
                         is described above. 
                         
        @rtype: List of Lists
        @return: This will simply turn the csv file to a list of lists where the
                 elements of the outer list are rows in the csv file. Each row is a list 
                 of all the columns.  The time delay will be converted to a float
                 while all other columns are converted to an integer.
        """
        try:
            fileObj = open(filename,'rU')
        except:
 ...
 #       
        ...
        sniffType = True;
        """
        @todo: this is sort of a hack. I could not find out how to just get the first row of
               csv document so i had to do the file determination in the loop of rows. This
               could be better implemented.
        """
#        for row in reader:
#            # this determines the type
#            if( rownum == 1):
#                if( row[0] == "# Time     Error        Bytes 1-13" or len(row[0]) == 18):
#                    sniffType = True;
#                    continue
#                else:
#                    sniffType = False;
...
#            rownum += 1
 # for every row
    ...
    def readInjectedFileDEC(self,filename,startTime = None,endTime = None,id=None):
        ...
        injected onto the bus. An example of a method that does this would be
        the experiments method L{experiments.generationFuzzer}. The file is assumed to 
        be a csv file with the .csv included matching this format. Any lines that
        begin with a # in the first column will be ignored. The user can specify start 
        and end times to only get a subset of the packet. If no startTime is provided 
        then it will return packets starting from the begining up until the endTime. 
        If no endTime is provided then it will return all packets in the file after the 
        startTime. An id can be provided and it will return only the packets with the given id.  
        
        When parsing the data it will assume the data is of the following form where 
        the first column is parsed as a float and all subsequent are parsed as a decimal ineteger:
        ...
        @type filename: String
        @param filename: path/filenae to the csv file to be uploaded. This is expected 
                         to include the .csv ending. The format of the rows
                         is described above.
        @type startTime: timestamp
        @param startTime: This is a timestamp of the start time for the earliest time 
                          for packets you want. The method will not return any packets that were injected
                          before this start time. This input is optional.
                         
        @type endTime: timestamp
        @param endTime: This is the timestamp of the latest inject time for packets you want.
                        The method will return no packets with an endTime after this timestamp.
                        This input is optional.
                        
        @type id: Integer
        @param id: This is an optional parameter that allows you to positively filter for the 
                   id that you want of packets. Only packets with the given id will be returned.
        
        @rtype: List of Lists
        @return: This imports the data on the csv file. Each element in the list will correspond
                 to a row. each element in the row will correspond to the column. The types of the 
        ...
        for row in reader:
            ...
            # if the user specified an end time to the packets that were fuzzed make sure it is before that.
                ...
                break
                # We are assuming that the file is ordered by time in an ascending order so that as soon as we are
            ...
            for col in row:
                ...
                if(colnum == 0):
                    ...
                    # if the difference between this packet's inject time is less than the
                    #  minimum and not more than 
                    # the maximum delay. The reason for the maximum delay is to make sure 
                    #that if you are combining
                    # different fuzzes you are not having an unwanted long pause
                    if( rownum != 0 and (timeInject-timePrev) > self.MIN_TIME_DELAY and \
                                            (timeInject-timePrev) < self.MAX_TIME_DELAY):
    ...
    def uploadFiles(self):
        """
        This method will upload all data files from sniffing experiments that have been saved.
        The sniff would be called by L{GoodFETMCPCANCommunication.sniff}
        and it will be all .csv files saved in the self.DATALOCATION which is input by the 
        user when the class is initiated. 
        
        This method will find all .csv files in the folder and attemp to upload them all. 
        It will call the L{uploadData} method on all the filenames. Once a 
        file has been uploaded (or attempted to) the file will be moved to a subfolder that
        is named by todays date in the following format: YYYYMMDD. If the
        folder does not exist, it will be created. The filename will then be changed by 
        adding  "_Uploaded" to the end of the csv. To ensure that no files are
        overwritten, the program will ensure that the file does not exist. if one does of
        the same filename it will append "_i" where i is the first integer that
        does not have a conflicting filename. This allows the user to upload multiple 
        times during the same day without loosing the origional data files.
        """
...
if __name__ == "__main__":
    ...
    """
    parser = \
    argparse.ArgumentParser(formatter_class=argparse.RawDescriptionHelpFormatter,description='''\
    ...
    if( verb == "test"):
        dm = DataManage(host="thayerschool.org", db="thayersc_canbus",username="thayersc_canbus",\
                                                            password="c3E4&$39",table="ford_2004")
        dm.testSQLPCAP()
    ...
    if( verb == "upload"):
        ...
        if( filename == None or table  == None):
            ...
            exit()
        dm = DataManage(host="thayerschool.org", db="thayersc_canbus",username="thayersc_canbus",\
                                                                    password="c3E4&$39",table=table)
        dm.uploadData(filename)
        
    # This will automatically upload all the csv files in the ../ThayerData/ folder. 
    # The uploaded files will be moved inot a
