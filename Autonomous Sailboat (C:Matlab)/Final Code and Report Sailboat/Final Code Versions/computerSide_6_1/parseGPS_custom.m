...
% abbreviated amount of data compared to the Garming GPS string that is
...
function [out, GPS_Location] = parseGPS_custom(gpsString, GPS_Location)
    ...
    %time stamp
    GPS_Location.timestamp = str2num(gpsString(breakInd(1)+1:breakInd(2)-2)); % extra -1 on the end is because it has a + at the end of the string, unknown reason (memcpy in C issue)
