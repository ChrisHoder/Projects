...
% Last Modified by GUIDE v2.5 28-May-2013 17:56:46
...
%value = get(handles.minX,'String')
    ...
    GPS_Location= struct('timestamp',0','lat',0,'long',0,'latD','N','longD','W','sog',0,'tog',0);
    setappdata(hMainGui,'GPS_Location',GPS_Location);
    boat_State = struct('rAngle',0,'sAngle',0,'desiredHeading',0,'control',0);
    ...
    while(1),
       ...
       % count = count + 1;
       %%% write data
%        rA_s2 = str2num(get(handles.rA_s,'String'));
%        if( rA_s2 ~= rA_s),
%           rA_s = rA_s;
%           fprintf(s,sprintf('$R,%d',round(rA_s)));
%        end
%        
%        sA_s2 = str2num(get(handles.sA_s,'String'));
%        if( sA_s2 ~= sA_s ),
%            sA_s = sA_s2;
...
%        
%        
% %        bot_state2 = str2num(get(handles.bot_state,'String')); %#ok<*ST2NM>
% %        
% %        if( bot_state2 ~= bot_state ),
% %           bot_state = bot_state2;
% %           fprintf(s,sprintf('$B,%d',bot_state));
% %        end
% %        
       ...
       if(~isempty(line)),
           ...
           elseif( strcmp(line(1:6),'$GPS')),
               ...
               compass(x,y);
               guidata(hObject, handles);
               set(handles.gpsStatus,'BackgroundColor','grey');
...
function desiredHeading_Callback(hObject, eventdata, handles)
% hObject    handle to desiredHeading (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of desiredHeading as text
%        str2double(get(hObject,'String')) returns contents of desiredHeading as a double


% --- Executes during object creation, after setting all properties.
function desiredHeading_CreateFcn(hObject, eventdata, handles)
% hObject    handle to desiredHeading (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% not in user control
if(boat_State.control ~= 1)
    return 
end

fprintf(s,sprintf('$R,%d',boat_State.rA_s-1));
boat_State.rA_s = boat_State.rA_s - 1;
setappdata(hMainGui,'boat_State',boat_State);
...
fprintf(s,sprintf('$S,%d',boat_State.sA_s+1));
boat_State.sA_s = boat_State.sA_s + 1;
setappdata(hMainGui,'boat_State',boat_State);
...
% handles    structure with handles and user data (see GUIDATA)
    ...
    heading = str2num(get(handles.desiredHeading,'String'));
