...
% Last Modified by GUIDE v2.5 29-May-2013 11:53:24
...
%value = get(handles.minX,'String')
    ...
    boat_State = struct('rAngle',0,'sAngle',0,'desiredHeading',0,'control',0,'windDir',0,'windVel',0);
...
fprintf(s,sprintf('$R,%d',boat_State.rAngle-1));
boat_State.rAngle = boat_State.rAngle - 1;
setappdata(hMainGui,'boat_State',boat_State);
...
fprintf(s,sprintf('$S,%d',boat_State.sAngle+1));
boat_State.sAngle = boat_State.sAngle + 1;
setappdata(hMainGui,'boat_State',boat_State);
...
% handles    structure with handles and user data (see GUIDATA)
    ...
    tempdata=get(handles.desiredHeading,'String');
