 ...
 *
 */
public class SimObjectList extends SimObject
{
	...
	{
		// doesn't matter won't be drawn or anything
		super(0,0);
		this.head = null;
		this.tail = null;
		this.count = 0;
	...
	@Override
	/**
	 * This method will update the state on the entire list. If the list is empty it will return false and be removed from the
	 * landscape. Otherwise it 2 two things. If the list is a list of Millipede's it will search down the list to see
	 * if any are dead. if they are dead it will cut the list at that Millipede. The dead millipede will become an obstacle and
	 * then there will now be 2 millipede segments.
	 * @param scape - Landscape being played on.
	 * @return true if there are elements in this list, false otherwise
	 */
	public boolean updateState(Landscape scape) {
		...
		//for the Millipede's check first to see if any died and turn them into obstacles. Since
		// we can only have one bullet on the landscape at a time, we can stop after the first dead Millipede is found
		if(( this.head.data instanceof Millipede) ){
			Node temp = this.head;
			// for each millipede in this list
			for( int i = 0 ; i < this.size() ; i++ ){
				Millipede mp = (Millipede)temp.data;
				// if the Millipede is dead, we need to cut the list at this point and create 2 millipede segments.
				// This dead millipede will also be replaced with an Obstacle object on the Landscape.
				if(!mp.isAlive()){
					SimObjectList newMillipede = this.Cut(i);
					SimObject  temp2 = newMillipede.remove(0);
					// add a new obstacle at this position
					scape.addAgent(new Obstacle(temp2.getX(), temp2.getY()));
					// add our new Millipede
					if(newMillipede.size() != 0){
						scape.addResource(newMillipede);
					}
					// this list is now empty after this element because we just cut it
					break;
				}
				
				temp = temp.next;
				// if there is no other elements, break from the list
				if(temp == null){
					break;
				}
			}
		...
		// update all of the millipede's in this list now (move them)
		this.updateAll(scape);
		// always return true if not empty
		return true;
	}

	@Override
	/**
	 * This method is empty. The SimObjectList cannot be drawn
	 */
