...
public class LandscapeDisplay extends JFrame
{
	...
	private class LandscapePanel extends JPanel
	{
		...
		public void paintComponent(Graphics g)
		{
			...
			SimObject agent;
			List resources = scape.getResources();
			...
			/* These 2 for loops were added so that it will try to paint the other simulation
			 * data if there are populated lists
			 */
			List socialAgents = scape.getSocialAgents();
			for (int i= 0; i < socialAgents.size(); i++){
				agent = (SimObject) socialAgents.get(i);
				agent.draw(g, (int) (agent.getX() * scale), (int) (agent.getY() * scale), scale);
			}
			
			List cells = scape.getCellList();
			for( int i=0 ; i < cells.size() ; i++){
				agent = (SimObject) cells.get(i);
				agent.draw(g, (int) (agent.getX() * scale), (int) (agent.getY() * scale), scale);
			}
		}
	}
	...
	{
		Landscape scape = new Landscape(40, 60);
		scape.addAgent(new SocialAgent(20, 20, 0));
		scape.addAgent(new SocialAgent(21, 20, 2));
		scape.addAgent(new SocialAgent(22, 20, 4));
