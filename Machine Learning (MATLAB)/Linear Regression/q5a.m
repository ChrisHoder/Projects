...
% (a) [14 points] The performance of the algorithm will depend on the 
% choice of the parameter ?. Tune this parameter via 10-fold cross 
% validation using ? ? {10?5,10?3,10?1,10,103, 105,107} for both the
% linear model and the quadratic model. Plot the cross-validation scores 
% (computed as the average of the mean squared errors obtained over the 10 
% validation sets) as a function of ? for both models.
...
for q = 1:2,
    ...
    %avgErrorTrain = zeros(size(lambdas,2),1);
