...
% Final Project
%
% This code is modified from the one we handed in for homework.
%
%
...
%
%
%  This method will first determine the non-degenerate features left and
%  then choose a random subset of 3 features before considering the best
%  split.
%
%
...
idxs = randperm(length(feat_idx),10000);
feat_Available = feat_idx(idxs,:);
% There are no possible improvements left. make this a leaf
% if(isempty(feat_remaining))
%     new_tree = tree;
%     % by majority vote determine what the label in this leaf will be
%     labelCount  = sumY/N;
%     
%     if( labelCount >= 0.5 ), % determine the leaf classification
%         label = 1;
%     else
%         label = 0;
%     end
...
%   
%     % create a new row in new_tree(slot, :), and fill in this row.
%     new_tree(slot,:) = [0 label labelCount];
%     % Return new_tree and new_slot, which is slot+1;
%     new_slot = slot + 1;
%     return; 
%     elseif( lenFeat <= 3),
%         feat_Available = feat_remaining;
%     elseif(lenFeat > 3),
%         feat_Available = feat_remaining(randperm(lenFeat,3));
% 
